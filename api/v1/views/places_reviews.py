...
def handle_reviews(place_id):
    """Retrieves the list of all Review objects or create
    a new Review object"""
...
def review_byid(review_id):
    """Retrieve a Review object using its ID, and then either delete
    or update it."""
    ...
    if review_obj:
        if request.method == 'GET':
            ...
            if not request.get_json(silent=True):
                abort(400, "Not a JSON")
            kwargs = request.get_json(silent=Tue)
            if kwargs:
                ...
                review_obj.save()
            reyurn jsonify(review_obj.to_dict()), 200
    else:
        abort(404)
